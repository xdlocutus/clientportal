...
?>
<div class="d-flex justify-content-between align-items-center mb-3"><h1 class="h3 mb-0">Quotes & Invoices</h1><?php if (has_permission('invoices.create')): ?><a class="btn btn-primary" href="/modules/invoices/add.php">Create Quote</a><?php endif; ?></div>
<div class="card border-0 shadow-sm"><div class="table-responsive"><table class="table table-striped mb-0"><thead><tr><th>Invoice #</th><th>Client</th><th>Type</th><th>Date</th><th>Due</th><th class="text-end">Total</th><th class="text-end">Paid</th><th class="text-end">Balance</th><th>Status</th><th></th></tr></thead><tbody><?php foreach ($invoices as $invoice): $balance = invoice_balance_amount((float) $invoice['total_amount'], (float) $invoice['paid_amount']); ?><tr><td><?= h($invoice['invoice_number']) ?></td><td><?= h($invoice['company_name']) ?></td><td><span class="badge text-bg-light border"><?= h(format_billing_type((string) ($invoice['billing_type'] ?? 'once_off'))) ?></span></td><td><?= h($invoice['invoice_date']) ?></td><td><?= h($invoice['due_date']) ?></td><td class="text-end"><?= h(money_format_portal((float) $invoice['total_amount'])) ?></td><td class="text-end"><?= h(money_format_portal((float) $invoice['paid_amount'])) ?></td><td class="text-end fw-semibold"><?= h(money_format_portal($balance)) ?></td><td><?= invoice_status_badge($invoice['status']) ?></td><td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="/modules/invoices/view.php?id=<?= (int) $invoice['id'] ?>">View</a><?php if (has_permission('invoices.edit')): ?> <a class="btn btn-sm btn-outline-primary" href="/modules/invoices/edit.php?id=<?= (int) $invoice['id'] ?>">Edit</a><?php endif; ?><?php if (has_permission('invoices.delete')): ?> <a class="btn btn-sm btn-outline-danger" href="/modules/invoices/delete.php?id=<?= (int) $invoice['id'] ?>" data-confirm="Delete this invoice?">Delete</a><?php endif; ?></td></tr><?php endforeach; ?><?php if (!$invoices): ?><tr><td colspan="10" class="text-center text-muted py-4">No invoices found.</td></tr><?php endif; ?></tbody></table></div></div>
