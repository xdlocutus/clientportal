...
$isQuote = invoice_is_quote($invoice);

if (is_post() && request_string('action') === 'convert_to_invoice') {
    require_permission('invoices.edit');
    verify_csrf();

    if ($isQuote) {
        $newStatus = ((string) $invoice['due_date'] < date('Y-m-d')) ? 'overdue' : 'unpaid';
        db()->prepare('UPDATE invoices SET status = :status, updated_at = NOW() WHERE id = :id')->execute([
            'id' => (int) $invoice['id'],
            'status' => $newStatus,
        ]);
        set_flash('success', 'Quote converted to invoice successfully.');
    }
...
$pageTitle = $isQuote ? 'Quote Details' : 'Invoice Details';
require BASE_PATH . '/includes/header.php';
?>
<div class="card border-0 shadow-sm"><div class="card-body">
<div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4"><div><h1 class="h3 mb-0"><?= h($invoice['invoice_number']) ?></h1><div class="text-muted"><?= h($invoice['company_name']) ?></div></div><div class="d-flex flex-wrap align-items-center gap-2"><?php if ($isQuote && has_permission('invoices.edit')): ?><form method="post" class="d-inline"><?= csrf_field() ?><input type="hidden" name="action" value="convert_to_invoice"><button class="btn btn-primary">Convert to Invoice</button></form><?php elseif (!$isQuote): ?><a class="btn btn-outline-secondary" href="/modules/billing/index.php">Open Billing Centre</a><?php endif; ?><div><?= invoice_status_badge($invoice['status']) ?></div></div></div>
<div class="row mb-4"><div class="col-md-6"><p class="mb-1"><strong>Client:</strong> <?= h($invoice['contact_name']) ?></p><p class="mb-1"><strong>Email:</strong> <?= h($invoice['email']) ?></p><p class="mb-1"><strong>Billing Type:</strong> <?= h(format_billing_type((string) ($invoice['billing_type'] ?? 'once_off'))) ?></p></div><div class="col-md-6 text-md-end"><p class="mb-1"><strong><?= $isQuote ? 'Quote Date' : 'Invoice Date' ?>:</strong> <?= h($invoice['invoice_date']) ?></p><p class="mb-1"><strong><?= $isQuote ? 'Valid Until' : 'Due Date' ?>:</strong> <?= h($invoice['due_date']) ?></p><p class="mb-1"><strong><?= $isQuote ? 'Quoted Total' : 'Outstanding Balance' ?>:</strong> <?= h(money_format_portal($isQuote ? (float) $invoice['total_amount'] : $balanceAmount)) ?></p></div></div>
...
<?php if (!$isQuote): ?><div class="row g-4 mt-2">
    <div class="col-lg-7">
